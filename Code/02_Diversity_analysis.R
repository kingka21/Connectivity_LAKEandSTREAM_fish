...
#ANOVA 
hist(allecos$exp_rich) # seems like lots of studies dont transform richness 
anova(lm(exp_rich ~ CLASS, data = allecos))
LSDout<-LSD.test(allecos$exp_rich, allecos$CLASS, 1406, 46.23, alpha=0.05, p.adj = "bonferroni")  #specify the DF and MSE of the residuals
...
alpha_box<-ggplot(allecos, aes(x = CLASS, y = exp_rich, color = Type)) +
  geom_boxplot() +
...
LSDout<-LSD.test(simple.fit$resid, waterbodies_w_density$CLASS, 1204, 44.56, alpha=0.05, p.adj = "bonferroni")  #specify the DF and MSE of the residuals

#add the residuals to the original dataset 
resid<-simple.fit$resid
waterbodies_w_density$resid<-resid

waterbodies_w_density$CLASS <- ordered(waterbodies_w_density$CLASS, c("HW_stream", "ISO_lake", "HW_lake", "DR_lake",  "MID_stream", "DR_lake_lake", "RIVER"))

resid_box<-ggplot(waterbodies_w_density, aes(x = CLASS, y = resid, color = Type)) +
  geom_boxplot() +
  scale_color_manual(values = c("purple", "orange"  )) + 
  labs(x = NULL) +
...
Fig3<-cowplot::plot_grid(alpha_box, resid_box,
  align = 'vh',
  labels = c("A", "B")
)
